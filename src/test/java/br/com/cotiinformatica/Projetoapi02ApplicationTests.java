...
class Projetoapi02ApplicationTests {
	...
	public void postLoginTest() throws Exception {
		...
		mockMvc.perform(
				post("/api/register").contentType("application/json").content(objectMapper.writeValueAsString(dtoRegister)));
	...
	public void postPasswordTest() throws Exception {
		...
		mockMvc.perform(
				post("/api/password").contentType("application/json").content(objectMapper.writeValueAsString(dtoPassword)))
				.andExpect(status().isOk());
	}
